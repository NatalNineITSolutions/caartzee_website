...
const SalesServices = () => {
  // Pull sales-specific services from your markdown data
  const salesServicesData = getMarkDownData<IService & { [key: string]: unknown }>('src/data/services').slice(9, 13);
        ...
        <div className="space-y-[40px] md:space-y-[50px] lg:space-y-[60px] xl:space-y-[70px]">
          ...
          <div className="mx-auto max-w-[810px] space-y-5 text-center lg:w-full">
            <RevealAnimation delay={0.1}>
              <span className="badge badge-green mb-3 md:mb-4 lg:mb-5">Sales Services</span>
            </RevealAnimation>
            <div className="space-y-3">
              <RevealAnimation delay={0.2}>
                <h2>Smart sales solutions built for Caartzee sellers.</h2>
              </RevealAnimation>
              <RevealAnimation delay={0.3}>
                <p className="mx-auto max-w-[582px] sm:w-full">
                  From marketplace integration to AI-powered analytics, Caartzee provides everything you need to sell smarter and scale effortlessly.
                </p>
              </RevealAnimation>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-center gap-8">
            {salesServicesData.map((service, idx) => (
              <RevealAnimation key={service.slug} delay={0.4 + idx * 0.1}>
                <article>
                  <div className="bg-background-2 dark:bg-background-5 flex w-full max-w-[403px] flex-col items-center gap-6 rounded-[20px] px-6 py-8 transition-transform duration-500 ease-in-out hover:translate-y-[-10px]">
                    ...
                    <div>
                      <LinkButton
                        href={`/sales-services/${service.slug}`}
