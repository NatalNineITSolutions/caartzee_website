import Image from 'next/image';
import Marquee from 'react-fast-marquee';
import RevealAnimation from '../animation/RevealAnimation';

type Logo = {
  src: string | string[];
  alt: string;
  className?: string;
  isDark?: boolean;
};

const salesLogos: Logo[] = [
  {
    src: '/images/icons/amazon-1.svg',
    alt: 'Amazon logo',
    className: 'ml-8',
    isDark: false,
  },
  {
    src: '/images/icons/flipkart.svg',
    alt: 'Flipkart logo',
    isDark: false,
  },
  {
    src: '/images/icons/shopify.svg',
    alt: 'Shopify logo',
    isDark: false,
  },
];

const SalesIntegrationMarquee = () => {
  return (
    <RevealAnimation delay={0.4}>
      <div className="space-y-7">
        {/* marquee row one */}
        <div className="relative mx-auto max-w-[1128px]">
          <div className="from-background-2/90 dark:from-background-7 absolute top-0 left-0 z-40 h-[115px] w-[15%] bg-gradient-to-r to-transparent md:w-[20%]" />
          <div className="from-background-2/90 dark:from-background-7 absolute top-0 right-0 z-40 h-[115px] w-[15%] bg-gradient-to-l to-transparent md:w-[20%]" />

          <Marquee autoFill speed={40}>
            <div className="flex items-center justify-center gap-[34px]">
              {salesLogos.map((logo, idx) => (
                <figure
                  key={idx}
                  className={`dark:bg-background-6 flex size-[100px] items-center justify-center rounded-full bg-white p-6 ${logo.className || ''}`}
                >
                  <Image
                    src={Array.isArray(logo.src) ? logo.src[0] : logo.src}
                    alt={logo.alt}
                    loading="lazy"
                    width={100}
                    height={100}
                    className="size-12 object-cover"
                  />
                </figure>
              ))}
            </div>
          </Marquee>
        </div>

        {/* marquee row two */}
        <div className="relative mx-auto max-w-[985px]">
          <div className="from-background-2/90 dark:from-background-7 absolute top-0 left-0 z-40 h-[115px] w-[15%] bg-gradient-to-r to-transparent md:w-[20%]" />
          <div className="from-background-2/90 dark:from-background-7 absolute top-0 right-0 z-40 h-[115px] w-[15%] bg-gradient-to-l to-transparent md:w-[20%]" />

          <Marquee autoFill speed={40} direction="right">
            <div className="flex items-center justify-center gap-[34px]" aria-label="Sales integration partners row 2">
              {salesLogos.map((logo, idx) => (
                <figure
                  key={idx}
                  className={`dark:bg-background-6 flex size-[100px] items-center justify-center rounded-full bg-white p-6 ${logo.className || ''}`}
                >
                  <Image
                    src={Array.isArray(logo.src) ? logo.src[0] : logo.src}
                    alt={logo.alt}
                    loading="lazy"
                    width={100}
                    height={100}
                    className="size-12 object-cover"
                  />
                </figure>
              ))}
            </div>
          </Marquee>
        </div>
      </div>
    </RevealAnimation>
  );
};

SalesIntegrationMarquee.displayName = 'SalesIntegrationMarquee';
export default SalesIntegrationMarquee;
